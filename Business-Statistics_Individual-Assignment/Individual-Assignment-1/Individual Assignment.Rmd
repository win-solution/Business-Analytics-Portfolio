...
```
We read the data of children in low income homes provided in csv format using read function. We group those data per borough and create a summary table that shows the average number of children, the standard deviation, the minimum number of children, and the maximum number of children across all years and wards in the dataset.



### Exclude unusual boroughs
```{r}
# Filter and exclude the boroughs with unusual numbers
...
```
We exclude a small number of boroughs that are unusual by using filter function.
...
```
We check the type of data using str function and try to find if there is any missing value in the data using summary function. Based on summary, no missing value has been found.
...
```
```{r}
# Use histogram to see if the distribution of the data is normal
...
This report is presenting specifically the data of children living in low income households and intended to be used by the policy makers or local government taking care the condition of prosperity level of households in London. The dataset used in this report is taken from 2014 to 2021 (8 years).
...
Based on the plot in Figure 1, the trend is up and down. The average number of children living in low income households was increasing from 2014 to 2019 but it was lowering down in 2020 and 2021. The number of children in low income households distributed around the mean in 2014, but it slightly distributed further away from the mean across the years until 2019. The number of children in low income households was starting to distribute near the mean again in 2020.

By using t-test to compare the dataset in the earliest year (2014) and the latest year (2021), we found that the mean of children in in low income households  2014 is 528 and the mean of children in low income households in 2021 is 603. The mean of children is significantly higher for year 2021, Welch $t(1095)=-4.02$, $p<.0001$, with a difference of 75, 95% confidence interval [-112 - -38.7].

The analysis conclusions using t-test contain deficiencies that need to be improved. The data used in for t-test can be considered positively skew distributed. Even though the majority of data the lies < 1000, there are some data having a number > 1000. This makes the t-test result is potentially biased and we should be cautious to interpret the data with a more appropriate analysis method.
...
# No. 3 (with geom_jitter)

# 
# ggplot(children_low_income_new,aes(x=year,y=children, color=year)) + 
#   geom_violin(scale="width") + 
#   geom_jitter(aes(group=year), position=position_jitterdodge(jitter.width = 1)) + 
#   stat_summary(fun.data=mean_sdl, fun.args = list(mult=1.5), geom="crossbar", 
#      width=0.5, position=position_dodge(),show.legend = FALSE) + 
#   theme(axis.text.x = element_text(angle = 45, margin=margin(0.5, unit="cm")))

# geom_jitter(position = position_jitter(seed = 1, width = 0.1)) +
# missing_values <- sum(is.na(children_low_income_new$children))


# outliers <- boxplot(children_low_income_for_ttest$children)$out
# children_low_income_for_ttest = filter(children_low_income_for_ttest,
#                                        !(children%in%c(outliers)))

# hist(children_low_income_for_ttest$children)
```
